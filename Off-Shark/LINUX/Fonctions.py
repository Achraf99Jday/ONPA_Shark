...
def OffsetChecker(liste):
    ...
    for i in range(len(res)):
        if(int(res[i][0],16) == 0):
            temp = int(res[i+1][0],16)
            res2.append(res[i][:temp+1])
        if(temp != 0 and int(res[i][0],16) !=0):    
            res2.append(res[i][:temp+1])
...
def FrameCheck(liste,Ethernet,IP,TCP):
    ...
    for i in range(NBt):
        if(len(Ethernet[i]) != 14):
            print("\nERROR: La trame N°"+str(i+1)+" présente une anomalie au niveau de l'entete ethernet (entete incomplète)")
            exec(open("./Off-Shark.py").read())
            exit()
        if(len(IP[i]) < 20):
            print("\nERROR: La trame N°"+str(i+1)+" présente une anomalie au niveau de l'entete IP (entete incomplète)")
            exec(open("./Off-Shark.py").read())
            exit()
        if(IP[i][9] == "06" and len(TCP[i]) < 20):
            print("\nERROR: La trame N°"+str(i+1)+" présente une anomalie au niveau de l'entete TCP (entete incomplète)")
            exec(open("./Off-Shark.py").read())
            exit()
